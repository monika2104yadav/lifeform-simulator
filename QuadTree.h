...
public:
	...
	void insert(const Obj&, const Point& pos,
		Procedure0 run_upon_resize = Procedure0());
...
template <class Obj>
class TreeNode {
	typedef std::pair<bool, Procedure0> Result;
	...
	Procedure0 resize_event;      // a callback that should be invoked when
...
public:
	...
	Procedure0 get_callbk(void) const { return resize_event; }
	...
	bool insert(const Obj& newobj, const Point& pos, Procedure0 new_resize,
		Procedure0& invoke_this) {
	...
	bool remove(const Point& pos, Obj& oldobj, Procedure0& invoke_this) {
		if (!in_bounds(pos)) return false;
	...
	*/
		...
		find_leaf(const Point& pos, const TreeNode<Obj>* parent = (TNPtr)0)  const
	{
		...
		if (is_leaf()) return make_pair((TNPtr)this, (TNPtr)parent);
		else {
			for (unsigned k = 0; k < 4; k++) {
				if (child[k]->in_bounds(pos))
			...
			/* NOT REACHED */
			assert(0);
			return make_pair((TNPtr)0, (TNPtr)0);
		}
		/* NOT REACHED */
	}
...
double QuadTree<Obj>::distance_to_edge(const Point& pos, double course) const {
	...
	//cout<<endl<<"calculated distance to edge "<<xdist;
	//cout<<endl<<"calculated distance to edge "<<ydist;
...
void QuadTree<Obj>::update_position(const Point& pos_old,
	...
	if (leaf->in_bounds(pos_new)) { // case 1: no callbacks
		...
		leaf->obj_pos = pos_new;
										   ...
										   occupied sibling. */
		Procedure0 obj_callback = leaf->get_callbk();

		/* remove the object FROM THE LEAF (not from the root) to
		avoid collapsing levels in the tree */
		Obj obj;
		Procedure0 null_callback;   // must be null since removing from a leaf
